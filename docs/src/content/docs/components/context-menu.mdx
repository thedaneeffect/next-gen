---
title: Context Menu
description: A menu that appears on right-click, with support for nested submenus.
---
import ApiTable from "@components/api-table.astro";
import Preview from "@previews/context-menu.svelte";
import Features from "@components/features.astro";
import { Tabs, TabItem } from '@astrojs/starlight/components';

<Preview client:load />

## Features

<Features>
- 🖱️ Opens on right-click at cursor position
- ⌨️ Full keyboard navigation (Arrow keys, Home, End, Enter, Space)
- 🔤 Typeahead search - jump to items by typing
- 📁 Multi-level nested submenus via `createSub()`
- 🎯 Smart pointer tracking with safe polygon zones
- ⏱️ 100ms hover delay for submenus
- ♿ Full ARIA menu semantics
- 🎨 CSS animation support with `@starting-style`
</Features>

## Usage

<Tabs>
	<TabItem label="Builder">
```svelte
<script lang="ts">
	import { ContextMenu } from "melt/builders";

	const menu = new ContextMenu();

	const copyItem = menu.getItem({ onSelect: () => console.log('copy') });
	const pasteItem = menu.getItem({ onSelect: () => console.log('paste') });
	const deleteItem = menu.getItem({ onSelect: () => console.log('delete') });
</script>

<div {...menu.trigger}>Right-click here</div>

<div {...menu.content}>
	<button {...copyItem.attrs}>Copy</button>
	<button {...pasteItem.attrs}>Paste</button>
	<hr {...menu.separator} />
	<span {...menu.label}>Actions</span>
	<button {...deleteItem.attrs}>Delete</button>
</div>
```
	</TabItem>

	<TabItem label="Component">
```svelte
<script lang="ts">
	import { ContextMenu } from "melt/components";
</script>

<ContextMenu>
	{#snippet children(menu)}
		{@const copyItem = menu.getItem({ onSelect: () => console.log('copy') })}
		{@const pasteItem = menu.getItem({ onSelect: () => console.log('paste') })}

		<div {...menu.trigger}>Right-click here</div>

		<div {...menu.content}>
			<button {...copyItem.attrs}>Copy</button>
			<button {...pasteItem.attrs}>Paste</button>
		</div>
	{/snippet}
</ContextMenu>
```
	</TabItem>
</Tabs>

## Submenus

Create nested submenus using `menu.createSub()`:

```svelte
<script lang="ts">
	import { ContextMenu } from "melt/builders";

	const menu = new ContextMenu();
	const shareSubmenu = menu.createSub();
	const socialSubmenu = shareSubmenu.createSub(); // Nested submenu

	// Main menu items
	const copyItem = menu.getItem({ onSelect: handleCopy });

	// Share submenu items and trigger
	const shareTrigger = shareSubmenu.trigger;
	const emailItem = shareSubmenu.getItem({ onSelect: handleEmail });

	// Social submenu items and trigger
	const socialTrigger = socialSubmenu.trigger;
	const twitterItem = socialSubmenu.getItem({ onSelect: handleTwitter });
	const facebookItem = socialSubmenu.getItem({ onSelect: handleFacebook });
</script>

<div {...menu.trigger}>Right-click here</div>

<!-- Main Menu -->
<div {...menu.content}>
	<button {...copyItem.attrs}>Copy</button>
	
	<!-- Submenu trigger (inside main menu) -->
	<button {...shareTrigger.attrs}>
		Share
		<span>›</span>
	</button>
</div>

<!-- Share Submenu -->
<div {...shareSubmenu.content}>
	<button {...emailItem.attrs}>Email</button>
	
	<!-- Nested submenu trigger -->
	<button {...socialTrigger.attrs}>
		Social
		<span>›</span>
	</button>
</div>

<!-- Social Submenu (nested inside Share) -->
<div {...socialSubmenu.content}>
	<button {...twitterItem.attrs}>Twitter</button>
	<button {...facebookItem.attrs}>Facebook</button>
</div>
```

## Keyboard Navigation

| Key | Action |
|-----|--------|
| `ArrowDown` | Move to next item |
| `ArrowUp` | Move to previous item |
| `ArrowRight` | Open submenu (when on submenu trigger) |
| `ArrowLeft` | Close submenu, return to parent |
| `Home` | Move to first item |
| `End` | Move to last item |
| `Enter` / `Space` | Select highlighted item |
| `Escape` | Close menu |
| `a-z` | Typeahead - jump to matching item |

## Disabled Items

Pass `disabled` to `getItem()` to disable an item:

```svelte
{@const pasteItem = menu.getItem({ 
	onSelect: handlePaste,
	disabled: !hasClipboardContent 
})}
<button {...pasteItem.attrs}>Paste</button>
```

Disabled items:
- Receive `data-disabled` attribute for styling
- Are skipped during keyboard navigation
- Cannot be selected

## Animating the Menu

Use CSS transitions with `@starting-style` for smooth animations:

```css
.menu-content {
	opacity: 0;
	transform: scale(0.95);
	transition:
		opacity 0.15s ease-out,
		transform 0.15s ease-out,
		display 0.15s allow-discrete,
		overlay 0.15s allow-discrete;
}

.menu-content:popover-open {
	opacity: 1;
	transform: scale(1);
}

@starting-style {
	.menu-content:popover-open {
		opacity: 0;
		transform: scale(0.95);
	}
}
```

## Styling Highlighted Items

The currently highlighted item receives `data-highlighted`:

```css
.menu-item[data-highlighted] {
	background-color: rgb(243 244 246);
}
```

## Configuration

### Scroll Behavior

Control what happens when scrolling while the menu is open:

```svelte
<script lang="ts">
	const menu = new ContextMenu({
		scrollBehavior: 'prevent', // 'close' | 'prevent' | 'allow'
	});
</script>
```

- `'close'` (default) - Closes the menu when scrolling outside of it
- `'prevent'` - Prevents page scroll when scrolling outside of it  
- `'allow'` - Allows normal page scrolling

Scrolling inside the menu content is always allowed.

### Floating UI Configuration

Customize menu positioning using [Floating UI](https://floating-ui.com/) options:

```svelte
<script lang="ts">
	import { offset, flip } from '@floating-ui/dom';

	const menu = new ContextMenu({
		floatingConfig: {
			computePosition: {
				placement: 'right-start',
				middleware: [offset(10), flip()],
			},
		},
	});
</script>
```

### Keyboard Navigation Loop

Control whether keyboard navigation wraps around at boundaries:

```svelte
<script lang="ts">
	const menu = new ContextMenu({
		loop: false, // Default: true
	});
</script>
```

When `loop` is `true`, pressing `ArrowDown` on the last item moves to the first item, and vice versa.

### Close on Pointer Leave

Control whether the menu closes when the pointer leaves the content area:

```svelte
<script lang="ts">
	const menu = new ContextMenu({
		closeOnPointerLeave: true, // Default: false
	});
</script>
```

When `false` (default), the menu stays open until explicitly closed. The "close on moving away before entering" behavior is unaffected.

## Controlled State

Control the open state externally:

```svelte
<script lang="ts">
	import { ContextMenu } from "melt/builders";

	let isOpen = $state(false);

	const menu = new ContextMenu({
		open: () => isOpen,
		onOpenChange: (open) => (isOpen = open),
	});
</script>

<p>Menu is {isOpen ? 'open' : 'closed'}</p>

<div {...menu.trigger}>Right-click here</div>
<div {...menu.content}>...</div>
```

## API Reference

<ApiTable entry="ContextMenu" />
